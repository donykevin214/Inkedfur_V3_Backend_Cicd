...
const addCheckout = async (req: Request, res: Response) => {
    ...
    try {
        ...
        for (let i = 0; i < products.length; i++) {
            ...
            if (product.category === 'PRINTS') {
                const product_crop_object = product.crop_size_list.filter((crop_list) => crop_list.size === products[i].crop_size);
                product_crop_object ? price = product_crop_object[0].price : price = 0;
            ...
            const customer = await Statistic.findOne({
                creator_id: product.user_id, date: {
            ...
            await Checkout.create({ transactionId, buyer_email, buyer_username, seller_id: product.user_id, product_id: product._id, quantity: products[i].quantity, unitPrice: price, crop_size: products[i].crop_size, shippingInfo, shipTo: shipToInfo, totalPrice: Number(totalPrice) + Number(shippingInfo.price), subTotal, tip, product_sell_type: products[i].product_sell_type, royalty, royalty_status: 'UNPAID', checkout_status: 'PENDING' });
        }
...
const saveShippingAddress = async (req: Request, res: Response) => {
    const { first_name, last_name, email, address1, address2, city, phone_number, country, state, postcode } = req.body;
    const { user } = req;
    try {
        const user_id = user.id;
        const ship = await Shipping.findOne({ user_id });
        if (!ship) {
            await Shipping.create({ user_id, first_name, last_name, email, address1, address2, city, phone_number, country, state, postcode });
        } else {
...
const checkout = async (req: Request, res: Response, next: NextFunction) => {
    const { buyer_id, card, shippingInfo, billToInfo, shipToInfo, products, totalPrice }: CheckoutInterface = req.body;
    try {
        ...
        for (let index = 0; index < products.length; index++) {
            ...
            let price: number;
            if (product?.category === 'PRINTS') {
                const product_crop_object = product.crop_size_list.filter((crop_list) => crop_list.size === products[index]?.crop_size)
                product_crop_object ? price = product_crop_object[0].price : price = 0;
            } else {
            ...
            lineItem.setUnitPrice(price.toString());
            ...
            if (response != null) {
                if (response.getMessages().getResultCode() == APIContracts.MessageTypeEnum.OK) {
                    if (response.getTransactionResponse().getMessages() != null) {
                        log('Successfully created transaction with Transaction ID: ' + response.getTransactionResponse().getTransId());
                        log('Response Code: ' + response.getTransactionResponse().getResponseCode());
                        log('Message Code: ' + response.getTransactionResponse().getMessages().getMessage()[0].getCode());
                        log('Description: ' + response.getTransactionResponse().getMessages().getMessage()[0].getDescription());
                        req.transactionId = response.getTransactionResponse().getTransId();
                    }
                    else {
                        log('Failed Transaction.');
                        if (response.getTransactionResponse().getErrors() != null) {
                            log('Error Code: ' + response.getTransactionResponse().getErrors().getError()[0].getErrorCode());
                            log('Error message: ' + response.getTransactionResponse().getErrors().getError()[0].getErrorText());
                            return sendError(req, res, 400, response.getTransactionResponse().getErrors().getError()[0].getErrorText());
                        }
                    }
                }
                else {
...
export default { getSaleHistory, getPurchaseHistory, addCheckout, updateRoyaltyStatus, updateCheckoutStatus, getProductPrice, saveShippingAddress, getShippingAddress, checkout }