...
const getCartProduct = async (req: Request, res: Response) => {
    ...
    try {
        ...
        for (let index = 0; index < allProduct.length; index++) {
            ...
            const product_crop_object = productDetail.crop_size_list.filter((crop_list) => crop_list.size === allProduct[index].crop_size);
            cartList.push({
                ...
                price: product_crop_object.length > 0 ? product_crop_object[0].price : allProduct[index].product_sell_type === PRODUCT_SELL_TYPE.PHYSICAL ? productDetail.physical_price : productDetail.digital_price,
...
const addCartProduct = async (req: Request, res: Response, next: NextFunction) => {
    const { product_id, quantity, product_sell_type, crop_size } = req.body;
    const { user } = req;
    try {
        const user_id = user.id;
