...
const getCreatorByID = async (req: Request, res: Response) => {
    const { user_id } = req.body;
    try {
        const creator = await User.findOne({ user_id, roles: Roles.CREATOR, active: true, status: USER_STATUS.ACTIVATE });
...
const getCreators = async (req: Request, res: Response) => {
    ...
    try {
        const creators = await User.find({ roles: Roles.CREATOR, active: true, status: USER_STATUS.ACTIVATE }).limit(skip).skip((index - 1) * skip);
